...
export default class TidalProvider extends Provider {
    ...
    public async search(query: string, type?: TidalSearchType): Promise<string | undefined> {
        ...
        // quick fix, if there's a feature request for a better way to do this, please open an issue
        ...
        return res.included
            ...
            .attributes.externalLinks[0].href
    }

    public async getTrack(name: string, artist: string, id: string): Promise<string | undefined> {
        return this.search(`${name} ${artist}`, TidalSearchType.Tracks)
    }

    public async getAlbum(name: string, artist: string, id: string): Promise<string | undefined> {
        return this.search(`${name} ${artist}`, TidalSearchType.Albums)
    }

    public async getArtist(name: string, id: string): Promise<string | undefined> {

...
export default abstract class Provider {
  ...
  public getTrack(name: string, artist: string, _id: string): Promise<string | undefined> {
    if (!this.supportsType(ProviderType.Track)) return Promise.resolve(undefined);
    return this.search(`${name} ${artist}`);
  }
  public getAlbum(name: string, artist: string, _id: string): Promise<string | undefined> {
    if (!this.supportsType(ProviderType.Album)) return Promise.resolve(undefined);
    return this.search(`${name} ${artist}`);
  }
  public getArtist(name: string, _id: string): Promise<string | undefined> {

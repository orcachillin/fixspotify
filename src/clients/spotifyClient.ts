...
export default class SpotifyClient {
  ...
  public async getTrackEmbed(id: string): Promise<any> {
    ...
    return {
      ...
      description: [
        ...
        track.totalTracks == 1
          ? `On ${track.album} (Single)`
          : `Track ${track.trackNumber} of ${track.totalTracks} on ${track.album}`,
  ...
  public async getAlbumEmbed(id: string): Promise<any> {
    ...
    return {
      ...
      description: [
        ...
        "",
        (album.tracks?.length || 0) > 10 ? `${(album.tracks?.length || 0) - 10} more...` : "",
      ].join("\n"),
    };
  }
  ...
  public async getPlaylistEmbed(id: string): Promise<any> {
    ...
    const tracks = (await this.client.tracks.getMultiple(playlistTracks.map((track) => track.track!.id))).reduce(
      (acc, track) => {
        acc[track.id] = track;
        return acc;
      },
